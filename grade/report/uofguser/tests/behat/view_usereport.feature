...
Feature: We can use the UofG user report
  As a user
  I browse to the UofG User report
  ...
  Scenario: Verify we can view a UofG user grade report with no users enrolled.
    When I am on the "Course 1" "grades > UofG User Report > View" page logged in as "admin"
    Then I should see "There are no students enrolled in this course."

  Scenario: Teacher sees his last viewed user report when navigating back to the gradebook UofG user report.
    ...
    When I am on the "Course 1" "grades > UofG User Report > View" page
    Then I should not see "Search for a user to view their report" in the "region-main" "region"
    And I should see "Student 1" in the "region-main" "region"
    And I am on the "Course 1" "grades > UofG User Report > View" page logged in as "teacher2"
    And I should see "Search for a user to view their report" in the "region-main" "region"

  Scenario: Teacher sees his last viewed UofG user report if the user is a part of the the current group.
    ...
    When I navigate to "View > UofG User Report" in the course gradebook
  ...
  Scenario: Teacher does not see his last viewed UofG user report if the user is no longer enrolled in the course.
    Given the following "users" exist:
      | username | firstname | lastname | email                |
      | teacher1 | Teacher   | 1        | teacher1@example.com |
      | student1 | Student   | 1        | student1@example.com |
