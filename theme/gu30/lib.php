 ...
 * @return string
 */
function theme_gu30_extra_less($theme) {
 ...
 * @return bool
 */
function theme_gu30_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM && ($filearea === 'logo')) {
        $theme = theme_config::load('gu30');
...
function theme_gu30_bootstrap3_grid() {
 ...
 */
function theme_gu30_login_title($theme) {
...
function theme_gu30_login_description($theme) {
    return empty($theme->settings->logindescription) ? '' : $theme->settings->logindescription;
}