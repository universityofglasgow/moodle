...
class grade_capture_testcase extends advanced_testcase {
    ...
    public function test_import_grade() {
        ...
        $DB->set_field('grade_items', 'grademax', '22.00000', array('iteminstance' => $mod1->id,
         'itemmodule' => 'assign'));
        $DB->set_field('grade_items', 'grademax', '22.00000', array('iteminstance' => $mod2->id,
         'itemmodule' => 'assign'));

        // Adding invalid scale.
        $name = "Non-22 Scale";
        $scale = "0,1,2";
        $non22scale = $this->getDataGenerator()->create_scale(array('name' => $name, 'scale' => $scale,
         'courseid' => $this->course->id, 'userid' => $USER->id));

        // Creating invalid activity.
        $invalidmod = $gen->create_module('assign', array('name' => 'Invalid Assessment 1',
         'course' => $this->course->id));
        $DB->set_field('grade_items', 'scaleid', $non22scale->id, array('iteminstance' => $invalidmod->id,
         'itemmodule' => 'assign'));
        ...
        foreach ($gcatactivities as $act) {
            $moodlegi = grade_item::fetch(array('iteminfo' => $act->gradeitemid, 'itemtype' => 'manual',
             'itemname' => get_string('moodlegrade', 'local_gugcat')));
            $prvgi = grade_item::fetch(array('iteminfo' => $act->gradeitemid, 'itemtype' => 'manual',
    ...
    public function test_capture_admin_grades() {
        ...
        foreach ($this->students as $student) {
            ...
            $gradeobj->information = '1.00000';
            // First student grade = NS, 2nd student = MV.
            $gradeobj->rawgrade = ($student->id != $this->student->id) ? NON_SUBMISSION : MEDICAL_EXEMPTION;
        ...
        $gbgrades = grade_get_grades($this->course->id, 'mod', $cm->modname, $cm->instance,
         array($this->student->id, $this->student2->id));
    ...
    public function test_get_component_module() {
        ...
        $gc2a->parent = $gc1a->id;
        $gc2a->update();
        $categorygi = $DB->get_field('grade_items', 'id', array('courseid' => $this->course->id, 'itemtype' => 'category',
         'iteminstance' => $gc2a->id));
        $assignid = $DB->get_field('grade_items', 'id', array('courseid' => $this->course->id, 'itemmodule' => 'assign'));
