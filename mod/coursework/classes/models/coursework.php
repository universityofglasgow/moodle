...
class coursework extends table_base {
       ...
       // get only submissions that user can grade
        ...
        foreach ($submissions as $submission) {
            ...
            foreach ($files as $f) {
                /* @var $f stored_file */
                // remove _number
                $file = explode('.',$f->get_filename());
                $filename = $file[0];
                $extension = $file[1];
                $index = substr($filename,0,-2).'.'.$extension;

                $files_for_zipping[$index] = $f;
            }
        }
    ...
    /**
     * @param user $user
     * @return \local_ulcc_framework\table_base
     */
     ...
     * @return bool
     */
    public function due_to_send_first_reminders() {
        global $CFG;

        return ($this->get_deadline() - $CFG->coursework_day_reminder) < time();
    }

    /**
     * @return bool
     */
    public function due_to_send_second_reminders() {
        global $CFG;

        return ($this->get_deadline() - $CFG->coursework_day_second_reminder) < time();
    }
        ...
        if ($this->extensions_enabled() && $this->extension_exists()){
            ...
            foreach ($submissions_with_extensions as $submission){
                ...
                if ($submission->extended_deadline > time()){
                    $excludesubmissions[] = $submission->submissionid;
                }
            }
            // build exclude sql
