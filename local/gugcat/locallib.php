...
class local_gugcat {
    ...
    public static function set_grade_scale($scaleid = null, $scaletype = SCHEDULE_A){
        ...
        $scalegrades = array();
        if(is_null($scaleid)){
            if (!reset(self::$SCHEDULE_A) && !reset(self::$SCHEDULE_B)) {
                list($scalegrades, $schedB) = self::get_gcat_scale();
            } else {
                $scalegrades = self::$SCHEDULE_A;
                // Schedule B starts 1 => H
                $schedB = array_reverse(array_filter(array_merge(array(0), self::$SCHEDULE_B)),true);
            }
        ...
        if($gradehistory_arr > 0){
            ...
            foreach($gradehistory_arr as $key=>$gradehistory){
                ...
                $modby = (preg_match('/import/i', $gradehistory->feedback) || preg_match('/grade/i', $gradehistory->feedback)
                         || preg_match('/aggregation/i', $gradehistory->feedback) || preg_match('/systemupdatecourse/i', $gradehistory->feedback)) ? null : $DB->get_record('user', array('id' => $gradehistory->usermodified), $fields);
