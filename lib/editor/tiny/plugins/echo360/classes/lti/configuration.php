...
class configuration {
    ...
    const LTI_INSTRUCTOR_ROLES = array(
      ...
      self::LTI_INSTRUCTOR,
      self::LTI_TEACHER,
      self::LTI_EDITING_TEACHER,
      self::LTI_NON_EDITING_TEACHER,
      self::LTI_MANAGER,
      self::LTI_MENTOR,
    ...
    public function __construct(\context $context, \stdClass $course, $cm) {
        ...
        $this->launchurl = self::get_plugin_config('hosturl');
        $this->consumerkey = self::get_plugin_config('consumerkey');
        $this->secretkey = self::get_plugin_config('sharedsecret');

        $this->rolesarray = self::get_role_names($context, $course);
        ...
        if ($cm &&
                $cm->modname == 'assign' &&
                has_capability('mod/assign:submit', $context, $USER->id) &&
                !has_capability('mod/assign:grade', $context, $USER->id)) {
            $this->contentintendeduse = "homework";
        }
    }
     ...
     * @throws \Exception
     */
    public static function get_plugin_config($key) {
        $value = get_config('tiny_echo360', $key);
        if (empty($value)) {
            throw new Exception('Configuration Missing: ' . (string) $key);
        }
        return $value;
    }
    ...
    public static function get_role_names(\context $context, \stdClass $course) {
        ...
        if (empty($rolenames)) {
            throw new Exception('No user roles found for user: ' . $USER->id . '.');
        }
        return $rolenames;
    }
    ...
    public function sanitize_roles($roles) {
        ...
        $highestrole = self::LTI_ROLE_REQUEST_LEARNER;
        foreach ($roles as $role) {
            if (in_array($role, self::LTI_ADMIN_ROLES) || strpos($role, 'administrator') !== false) {
                $highestrole = self::LTI_ROLE_REQUEST_ADMINISTRATOR;
                break;
            } else if (in_array($role, self::LTI_INSTRUCTOR_ROLES) || strpos($role, 'instructor') !== false) {
                $highestrole = self::LTI_ROLE_REQUEST_INSTRUCTOR;
            }
        }
        return $highestrole;
    }
    ...
    public function generate($customparams = array()) {
        ...
        $launchdata = array(
            ...
            'ext_content_intended_use' => $this->contentintendeduse,
            ...
            'context_title' => $this->course->fullname,
            'context_label' => $this->course->shortname,
            'custom_echo360_plugin_version' => $this->version,
            'user_id' => $USER->id,
            ...
            'roles' => $this->sanitize_roles($this->rolesarray),
        ...
        $launchdata['oauth_signature'] = $this->get_oauth_signature($this->launchurl, $launchdata, $this->secretkey);
