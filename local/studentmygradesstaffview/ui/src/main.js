...
function ensureGUIsSet(timeout) {
    var start = Date.now();
    return new Promise(waitForSMGSV);


    function waitForSMGSV(resolve, reject) {
        if (window.SMGSV) {
            resolve(window.SMGSV)
        } else if (timeout && (Date.now() - start) >= timeout) {
            reject(new Error("timeout"));
        } else {
            setTimeout(waitForSMGSV.bind(this, resolve, reject), 30);
        }
    }
}
...
.then(() => {
    ...
    fetchMany([{
        methodname: 'local_studentmygradesstaffview_get_all_strings',
        args: {
        }
    }])[0]
    .then((result) => {
