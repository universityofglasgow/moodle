@gradereport @gradereport_uofguser
Feature: Double-check if we don't show hidden grades for users without the 'moodle/grade:viewhidden' capability on user report
  In order to show user report in secure way
  As a teacher without the 'moodle/grade:viewhidden' capability
  I should not see hidden grades in the UofG user report
  ...
  Background:
    ...
    And the following config values are set as admin:
      | grade_report_uofguser_showhiddenitems           | 2 |
      | grade_report_uofguser_showtotalsifcontainhidden | 2 |
  ...
  @javascript
  Scenario: View UofG user report containing hidden activities or grade items or grades with capabilities with student view
    Given I am on the "Course 1" "grades > UofG User Report > View" page logged in as "teacher1"
  ...
  @javascript
  Scenario: View UofG user report containing hidden activities or grade items or grades with capabilities with teacher view
    Given I am on the "Course 1" "grades > UofG User Report > View" page logged in as "teacher1"
  ...
  @javascript
  Scenario: View UofG user report containing hidden activities or grade items or grades without capabilities with teacher view
    Given I am on the "Course 1" "grades > UofG User Report > View" page logged in as "teacher1"
  ...
  @javascript
  Scenario: View UofG user report containing hidden activities or grade items or grades without capabilities with student view
    Given I am on the "Course 1" "grades > UofG User Report > View" page logged in as "teacher1"
    And the following "role capability" exists:
      | role                    | editingteacher  |
      | moodle/grade:viewhidden | prohibit        |
