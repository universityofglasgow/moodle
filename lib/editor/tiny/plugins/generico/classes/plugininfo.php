...
class plugininfo extends plugin implements plugin_with_configuration, plugin_with_buttons, plugin_with_menuitems
{
    ...
    public static function get_plugin_configuration_for_context(
        ...
        $tinygenericoconfig = get_config(constants::M_COMPONENT);
        $genericoconfig = get_config('filter_generico');
        $params = [];

        // On first install $config has nothing but "version", and the rest of this function would throw debug warnings
        // So we just cancel out here.
        if (!$genericoconfig || count(get_object_vars($genericoconfig)) < 1) {
            return $params;
        }
