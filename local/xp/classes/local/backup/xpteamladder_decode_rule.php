 ...
 */
class xpteamladder_decode_rule extends \restore_decode_rule {

    /**
     * Constructor.
     */
    public function __construct($placeholder = 'BLOCKXPSHORTCODEXPTEAMLADDER') {
    ...
    public function decode($content) {
        ...
        foreach ($matches[0] as $key => $tosearch) {
            ...
            if ($context) {
                $shortcode .= " ctx={$context->id} secret=" . handler::get_xpteamladder_secret($context);
            }
            if ($top) {
                $shortcode .= " top={$top}";
            }
    ...
    public static function encode_content($content) {
        ...
        $content = filter_shortcodes_process_text($content, function ($tag) {
            ...
            return (object) ['hascontent' => false, 'contentprocessor' => function ($args, $content) {
                $top = $args['top'] ?? 0;
                return '$@BLOCKXPSHORTCODEXPTEAMLADDER'
                    . '*' . ($args['ctx'] ?? '0')
                    . '*' . ($top === true ? '10' : $top)
