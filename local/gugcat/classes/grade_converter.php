...
class grade_converter{
     ...
     * @return int $grade
     */
    public static function convert($conversion, $grade, $isschedb = false) {
        ...
        foreach ($convs as $index => $cobj) {
            ...
            if ($isschedb && $upperbound && $grade <= $upperbound  && $grade > $cobj->lowerboundary) {
        ...
        if (is_null($convertedgrade)) {
            $gradecv = empty($convs) ? $grade : min(array_column($convs, 'grade'));
            $convertedgrade = empty($convs) ? $grade : ($gradecv == 0 ? 0 : intval($gradecv) - 1);
        }
