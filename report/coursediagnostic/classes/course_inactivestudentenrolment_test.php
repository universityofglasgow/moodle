...
/**
 * Are there inactive students enrolled on this course.
 *
 * This test checks for students that have been inactive on a course.
...
class course_inactivestudentenrolment_test implements \report_coursediagnostic\course_diagnostic_interface {
    ...
    public function runtest(): array {
        ...
        if ($enrolledusers == true) {
            if ($inactiveusers == false) {
                $outcometext = get_string('inactivestudentenrolment_success_text', 'report_coursediagnostic');
            } else {
                $options = [
                    ...
                    'word2' => (($counter > 1) ? get_string('plural_7', 'report_coursediagnostic') :
                        get_string('singular_7', 'report_coursediagnostic')),
                    'word3' => (($counter > 1) ? get_string('plural_2', 'report_coursediagnostic') :
                        get_string('singular_2', 'report_coursediagnostic'))
                ];
                $outcometext = get_string('inactivestudentenrolment_not_accessed_text', 'report_coursediagnostic', $options);
            }
        } else {
            $options = [
                'participantslink' => $participantslink,
            ];
            $outcometext = get_string('inactivestudentenrolment_no_enrolments_text', 'report_coursediagnostic', $options);
        }
