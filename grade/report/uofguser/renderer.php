 ...
 * $renderer = $PAGE->get_renderer('gradereport_uofguser');
 *
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradereport_uofguser_renderer extends plugin_renderer_base {
    ...
    public function view_user_selector(int $userid, int $userview): string {
        ...
        $options = [
            GRADE_REPORT_UOFGUSER_VIEW_USER => get_string('otheruser', 'grades'),
            GRADE_REPORT_UOFGUSER_VIEW_SELF => get_string('myself', 'grades'),
        ];
     ...
     * @return string The raw HTML to render.
     * @throws coding_exception
     * @deprecated since Moodle 4.5. See user_selector use in \gradereport_uofguser\output\action_bar::export_for_template.
     */
    public function users_selector(object $course, ?int $userid = null, ?int $groupid = null, string $usersearch = ''): string {
        ...
        $resetlink = new moodle_url('/grade/report/uofguser/index.php', ['id' => $course->id, 'group' => 0]);
        $baseurl = new moodle_url('/grade/report/uofguser/index.php', ['id' => $course->id]);
        $this->page->requires->js_call_amd('gradereport_uofguser/user', 'init', [$baseurl->out(false)]);
    ...
    public function user_navigation(graded_users_iterator $gui, int $userid, int $courseid): string {
        ...
        if ($keynumber !== 0) {
            ...
            $navigationdata['previoususer'] = [
                ...
                'url' => (new moodle_url('/grade/report/uofguser/index.php', ['id' => $courseid, 'userid' => $previoususer->id]))
                    ->out(false),
        ...
        if ($keynumber < count($users) - 1) {
            ...
            $navigationdata['nextuser'] = [
                ...
                'url' => (new moodle_url('/grade/report/uofguser/index.php', ['id' => $courseid, 'userid' => $nextuser->id]))
                    ->out(false),
                'nextarrow' => $nextarrow,
            ];
        }

        return $this->render_from_template('gradereport_uofguser/user_navigation', $navigationdata);
    }
     ...
     * @param int $courseid The course ID.
     * @return string The raw HTML to render.
     * @deprecated since Moodle 4.5 See select_menu use in \gradereport_uofguser\output\action_bar::export_for_template.
     */
    public function view_mode_selector(int $userid, int $userview, int $courseid): string {
        ...
        $viewasotheruser = new moodle_url('/grade/report/uofguser/index.php', ['id' => $courseid, 'userid' => $userid,
            'userview' => GRADE_REPORT_UOFGUSER_VIEW_USER]);
        $viewasmyself = new moodle_url('/grade/report/uofguser/index.php', ['id' => $courseid, 'userid' => $userid,
            'userview' => GRADE_REPORT_UOFGUSER_VIEW_SELF]);
        ...
        $selectoractiveurl = $userview === GRADE_REPORT_UOFGUSER_VIEW_USER ? $viewasotheruser : $viewasmyself;
        ...
        return $this->render_from_template('gradereport_uofguser/view_mode_selector',
