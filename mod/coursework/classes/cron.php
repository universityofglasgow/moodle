...
class cron {
    ...
    private static function send_reminders_to_students() {
        ...
        foreach ($raw_courseworks as $raw_coursework) {
            ...
            if (!$coursework->students_can_still_submit()) {
                continue;
            }

            if (!$coursework->due_to_send_first_reminders()) {
                continue;
            }
            ...
            foreach ($students as $student) {
                if ($student->has_not_been_sent_reminder($coursework, 1)) {
                    $userswhoneedreminding[$student->id()] = $student;
                }
            }

            if ($coursework->due_to_send_second_reminders()) {
                foreach ($students as $student) {
                    if (!array_key_exists($student->id(), $userswhoneedreminding) && $student->has_not_been_sent_reminder($coursework, 2)) {
                        $userswhoneedreminding[$student->id()] = $student;
                    }
                }
            }
    ...
    private static function send_email_reminders_to_students(array $users, array &$counts) {
        ...
        foreach ($users as $user) {
            ...
            // Make the deadline human-readable.
            ...
            $user->deadline = userdate($user->deadline,'%a, %d %b %Y, %H:%M');
            $user->day_hour = $days_to_deadline;

            $subject = get_string('cron_email_subject', 'mod_coursework', $user);
            $text = get_string('cron_email_text', 'mod_coursework', $user);
            $html = get_string('cron_email_html', 'mod_coursework', $user);

            // Log the send so we don't do it next time.
            // TODO don't use email, use the moodle messaging system as the users may have configured the system
            // to send messages via different channels.
            if (email_to_user($user, 'Course admin', $subject, $text, $html)) {

                $emailcounter++;
                if (!isset($usercounter[$user->userid])) {
                    $usercounter[$user->userid] = 1;
                } else {
                    $usercounter[$user->userid]++;
                }

                $email_reminder = new stdClass();
                $email_reminder->userid = $user->userid;
