...
class reformation_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('hidden', 'form', 'reformation');
        $mform->setType('form', PARAM_ALPHA);

        // Form elements.
        $mform->addElement('static', 'wi', '', "<h3>National Federation of Women's Institutes");
        $mform->addElement('static', 'formation', '', '<h3>Notice of Reformation of WI</h3>');
        ...
        $mform->addElement('date_selector', 'datewisuspended', 'Date WI suspended');

        $mform->addElement('date_selector', 'datereformationmeeting', 'Date of reformation meeting');

        $mform->addElement('text', 'previousname', "Previous name of WI if different", $att);
        $mform->setType('previousname', PARAM_TEXT);
    ...
    public function format_html( $data ) {
        ...
        $html .= "<h3>Notice of Reformation of a WI</h3>\n";
        $html .= '<table cellspacing="0" cellpadding="5" >';
        $html .= "<tr><th>Federation:</th><td>{$data->federation}</td></tr>\n";
        $html .= "<tr><th>Name of WI:</th><td>{$data->nameofwi}</td></tr>\n";
        $html .= "<tr><th>Date WI suspended:</th><td>" . userdate($data->datewisuspended, '%A, %e %B %G') . "</td></tr>\n";
        $html .= "<tr><th>Date reformation meeting:</th><td>" .
            userdate($data->datereformationmeeting, '%A, %e %B %G') . "</td></tr>\n";
        $html .= "<tr><th>Previous name if different:</th><td>{$data->previousname}</td></tr>\n";
