...
/**
 * The gradebook gusatffview user report
 *
...
require_once($CFG->dirroot.'/grade/report/uofguser/lib.php');

$courseid = required_param('id', PARAM_INT);
$userid   = optional_param('userid', null, PARAM_INT);
$userview = optional_param('userview', 0, PARAM_INT);

$PAGE->set_url(new moodle_url('/grade/report/uofguser/index.php', ['id' => $courseid]));

if ($userview == 0) {
    $userview = get_user_preferences('gradereport_uofguser_view_user', GRADE_REPORT_UOFGUSER_VIEW_USER);
} else {
    set_user_preference('gradereport_uofguser_view_user', $userview);
}

// Basic access checks (similar to core user report).
...
require_capability('gradereport/uofguser:view', $context);
...
// Access control (similar to core user report).
$access = false;
...
$USER->grade_last_report[$course->id] = 'uofguser';

// First make sure we have proper final grades.
// How can we adopt it for MyGrades?
...
if (has_capability('moodle/grade:viewall', $context)) {
    ...
    $lastvieweduserid = $SESSION->gradereport_uofguser["useritem-{$context->id}"] ?? null;
    ...
    if ($userview == GRADE_REPORT_UOFGUSER_VIEW_USER) {
        $viewasuser = true;
    } else {
        $viewasuser = false;
    }
    ...
    if (is_null($userid)) { // Zero state.
        $actionbar = new \gradereport_uofguser\output\action_bar($context, $userview, null, $currentgroup);
        // Print header.
        print_grade_page_head($courseid, 'report', 'uofguser', false, false, null, true,
            null, null, null, $actionbar);
        echo $OUTPUT->notification(
            get_string('custommessage:noone', 'gradereport_uofguser'),
            'notifymessage');
        ...
        if (empty($gradableusers)) { // There are no available gradable users, display a notification.
            ...
            $report = new gradereport_uofguser\report\uofguser($courseid, $gpr, $context, $USER->id, $viewasuser);
        ...
        $SESSION->gradereport_uofguser["useritem-{$context->id}"] = $userid;

        $actionbar = new \gradereport_uofguser\output\action_bar($context, $userview, 0, $currentgroup);
        print_grade_page_head($courseid, 'report', 'uofguser', false, false, null, true,
            null, null, null, $actionbar);
        echo $OUTPUT->notification(
            get_string('custommessage:allstudents', 'gradereport_uofguser'),
            'notifymessage');
        ...
        $report = new gradereport_uofguser\report\uofguser($courseid, $gpr, $context, $userid, $viewasuser);
        $actionbar = new \gradereport_uofguser\output\action_bar($context, $userview, $report->user->id, $currentgroup);

        print_grade_page_head($courseid, 'report', 'uofguser', false, false, false, true, null, null, $report->user, $actionbar);
        if ($viewasuser) {
            echo $OUTPUT->notification(
                get_string('custommessage:teacherasstudent', 'gradereport_uofguser'),
                'notifysuccess');
        } else {
            echo $OUTPUT->notification(
                get_string('custommessage:teacherasmyself', 'gradereport_uofguser'),
                'notifyproblem');
        }
        ...
        $userreportrenderer = $PAGE->get_renderer('gradereport_uofguser');
    ...
    $report = new gradereport_uofguser\report\uofguser($courseid, $gpr, $context, $userid ?? $USER->id);

    // Print the page.
    print_grade_page_head($courseid, 'report', 'uofguser', false, false, false, true, null, null, $report->user);
    echo $OUTPUT->notification(
        get_string('custommessage:student', 'gradereport_uofguser'),
        'notifymessage');
    if ($report->fill_table()) {
        echo $report->print_table(true);
    }
