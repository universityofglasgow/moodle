...
$gradinginfo = grade_get_grades($courseid, 'mod', $module->modname, $module->instance, $studentid);
...
$mform = new addeditgradeform(null, array('id' => $courseid, 'activity' => $module,
 'studentid' => $studentid, 'overview' => $overview));
if ($fromform = $mform->get_data()) {
    $gradereason = ($fromform->reasons == 9) ? $fromform->otherreason
    : local_gugcat::get_reasons()[$fromform->reasons];
    $grade = !is_numeric($fromform->grade) ? array_search(strtoupper($fromform->grade),
     local_gugcat::$grades) : $fromform->grade;
    $gradeitemid = local_gugcat::add_grade_item($courseid, $gradereason, $module);
    $notes = ",_gradeitem: $gradereason ,_notes: $fromform->notes";
    $grades = local_gugcat::add_update_grades($studentid, $gradeitemid, $grade, (!$isconverted ? $notes : ''));
    if ($isconverted) {
