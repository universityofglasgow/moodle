...
class grade_aggregation{
    ...
    public static function get_aggregated_grade($userid, $subcatobj, $gradeitems){
        ...
        if($pgobj){
            $updated = false;
            $autoscale = !is_null($scaleid) ? $scaleid : ($autoconverttob ? SCHEDULE_B : ($gradetype == GRADE_TYPE_SCALE ? SCHEDULE_A : null)); is_null($subcatobj->aggregation_type) ? $DB->set_field('grade_items', 'calculation', $subcatobj->aggregation, array('id'=>$pgobj->itemid)) : null;
