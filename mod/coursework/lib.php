...
function coursework_update_instance($coursework) {
    ...
    // Some fields are sometimes not sent by the form because they are disabled. They show up as the default
    // in this case instead of the previously set value, which then overwrites it. We need to unset
    // them apart from on create or when there is an actual value.
    $fieldstheformwontsend = array('deadline');
    foreach ($fieldstheformwontsend as $field) {

        $value = optional_param_array($field, null, PARAM_RAW);

        if (is_null($value)) {
            unset($coursework->$field);
        }
    }
