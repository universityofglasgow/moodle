...
class convertform extends moodleform {
    ...
    public function definition() {
        ...
        $isconverted = $activity->is_converted;
        $defaulttype = $isconverted ? $isconverted : SCHEDULE_A;
        ...
        $grades = grade_converter::process_defaults($activity->is_converted == SCHEDULE_A, local_gugcat::$schedulea, $existing);
        $schedb = grade_converter::process_defaults($activity->is_converted == SCHEDULE_B, local_gugcat::$scheduleb, $existing);
        ...
        $mform->addElement('select', 'scale', get_string('selectscale', 'local_gugcat'),
                 $scales, ['id' => 'select-scale', 'class' => 'mform-custom-select']);
        $mform->setType('scale', PARAM_NOTAGS);
        $mform->setDefault('scale', $defaulttype);
        $mform->addElement('select', 'template', get_string('selectprevconv', 'local_gugcat'),
                 $templates, ['id' => 'select-template', 'class' => 'mform-custom-select']);
        ...
        $mform->addElement('html', html_writer::tag('p', get_string('noteconversion', 'local_gugcat'),
                 array('class' => 'mt-3 font-weight-bold')));
    ...
    public function setup_table($grades, $mform, $name, $keys = array(), $maxgrade) {
        ...
        foreach ($grades as $index => $grd) {
            ...
            $mform->setDefault($name."[$index]", is_null($grd->lowerboundary) ? null
                    : floatval(grade_converter::convert_point_percentage($maxgrade, $grd->lowerboundary)));
            ...
            if ($index == 1) {
                ...
                $mform->addRule($name."[$index]", get_string('errorfielddecimal', 'local_gugcat'),
                         'regex', '/^[0-9]+(\.[0-9]{1,2})?$/', 'client');
                $mform->addRule($name."[$index]", get_string('errorfielddecimal', 'local_gugcat'),
                         'regex', '/^[0-9]+(\.[0-9]{1,2})?$/', 'server');
            }
            ...
            if ($index == 1) {
                ...
                $mform->addRule($name."_pt[$index]", get_string('errorfielddecimal', 'local_gugcat'),
                         'regex', '/^[0-9]+(\.[0-9]{1,2})?$/', 'client');
                $mform->addRule($name."_pt[$index]", get_string('errorfielddecimal', 'local_gugcat'),
                         'regex', '/^[0-9]+(\.[0-9]{1,2})?$/', 'server');
            }
